...
const validFields = ['Food', 'Sub_Food', 'FSA_Year']
...
export default async (req, res) => {
  ...
  if (field == 'FSA_Year') {
    values = await db.from('form_fsa_conventional').distinct(field).where(dependencies).orderBy(field, 'desc')
  } else {
    values = await db.from('form_fsa_conventional').distinct(field).where(dependencies).orderBy(field)
  }
